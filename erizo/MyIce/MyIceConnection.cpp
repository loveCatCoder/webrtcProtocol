...
void erizo::MyIceConnection::start()
{
    ...
    dzlog_info("m_nPort:%d", m_nPort);
    dzlog_info("m_strIp:%s", m_strIp.c_str());
...
void erizo::MyIceConnection::computeFingerprint(X509 *cert, char *fingerprint)
{
...
{
    // dzlog_info("ICE socket receive data");
...
int erizo::MyIceConnection::GetPort()
{
    return m_nPort;
}
